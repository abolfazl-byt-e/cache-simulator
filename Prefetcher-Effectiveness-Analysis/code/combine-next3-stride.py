...
def next_line_prefetch(cache, pred_mm_block_number, pred_set_number, policy):
    ''' block in cache checking...'''
...
def stride_prefetch(cache, pred_next_block, pred_set_number, policy):
    ''' block in cache checking...'''
    if pred_next_block in cache[f"s{pred_set_number}"]:
        ''' hit policy '''
        current_block = cache[f"s{pred_set_number}"].index(pred_next_block)
        element_to_move = cache[f"s{pred_set_number}"].pop(current_block)
        cache[f"s{pred_set_number}"].append(element_to_move)
    else:
        ''' miss policy '''
        ...
        if len(cache[f"s{pred_set_number}"]) < 8:
            ...
            replacement(policy, cache, pred_next_block, pred_set_number)
    return cache
        ...
        for benchmark in files:
            ...
            for policy in replacement_policies:
                ...
                with open(benchmark_path) as file:
                    ...
                    pred1_mm_block_number = pred2_mm_block_number = pred2_mm_block_number = -1
                    RPT = {-1 : [-1, -1]}
                    ...
                    for line in file: # read addresses line by line
                        ...
                        if missed:
                            pred1_mm_block_number = mm_block_number + 1
                            pred2_mm_block_number = mm_block_number + 2
                            pred3_mm_block_number = mm_block_number + 3
                            pred1_set_number = pred1_mm_block_number % number_of_sets
                            pred2_set_number = pred2_mm_block_number % number_of_sets
                            pred3_set_number = pred3_mm_block_number % number_of_sets
                            next_line_prefetch(cache, pred1_mm_block_number, pred1_set_number, policy)
                            next_line_prefetch(cache, pred2_mm_block_number, pred2_set_number, policy)
                            next_line_prefetch(cache, pred3_mm_block_number, pred3_set_number, policy)
                        elif mm_block_number == pred1_mm_block_number:
                            pred1_mm_block_number = mm_block_number + 1
                            pred2_mm_block_number = mm_block_number + 2
                            pred3_mm_block_number = mm_block_number + 3
                            pred1_set_number = pred1_mm_block_number % number_of_sets
                            ...
                            next_line_prefetch(cache, pred2_mm_block_number, pred2_set_number, policy)
                            next_line_prefetch(cache, pred3_mm_block_number, pred3_set_number, policy)
                        elif mm_block_number == pred2_mm_block_number:
                            pred1_mm_block_number = mm_block_number + 1
                            pred2_mm_block_number = mm_block_number + 2
                            pred3_mm_block_number = mm_block_number + 3
                            pred1_set_number = pred1_mm_block_number % number_of_sets
                            ...
                            next_line_prefetch(cache, pred3_mm_block_number, pred3_set_number, policy)
                        ''' stride prefetching...'''
                        value = RPT.get(pc, None)
                        if value != None:
                            last_address = value[0]
                            last_stride = value[1]
                            ...
                            if last_stride == current_stride:
                                ...
                                pred_set_number = pred_next_block % number_of_sets
                                cache = stride_prefetch(cache, pred_next_block, pred_set_number, policy)
                            else:
                                # update_RPT
                                RPT[pc] = [address, current_stride]
                        else:
                            RPT[pc] = [address, 0]
